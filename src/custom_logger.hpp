...
class Logger {
    ...
    private:
		...
		// Actual output
		template<class T>
		void write_out(const std::string& label, const T &msg){
			std::stringstream ss;
			ss << timestamp();
    ...
    public:
		...
		virtual ~Logger(){
			if(logfileSet)
				std::cout << "Execution log saved in " << logfile << std::endl;
		}
		
		template<class T>
		void info(const T &msg){ write_out("INFO", msg);}
		
		template<class T>
		void warn(const T &msg){ write_out("WARN", msg);}
		
		template<class T>
		void error(const T &msg){ write_out("ERROR", msg);}
